                                              ...
                                              float  connStrength)
{
    ...
    if (row != col) {
        cout << "* WARNING: GraphInputOutput::ReadMatrixMarketAdjacencyGraph()"
             << endl;
        cout << "*\t row!=col. This is not a square matrix. Can't process."
             << endl;
        return false;
    }
    ...
    while (!in.eof() &&
           entry_counter <
               num_of_entries)  // there should be (num_of_entries+1) lines in
                                // the input file (excluding the comments)
    {
        getline(in, line);
        entry_counter++;
        ...
        if (line != "") {
            ...
            if (rowIndex < 0 || rowIndex >= row)
                cout << "Something wrong rowIndex " << rowIndex << " row "
                     << row << endl;

            if (colIndex < 0 || colIndex >= col)
                cout << "Something wrong colIndex " << colIndex << " col "
                     << col << endl;
            ...
            // This is to handle directed graphs. If the edge is already
            // present, skip. If not add.
            int exists = 0;
    ...
    // cout << "No. of upper triangular pruned: " << num_upper_triangular <<
    // endl;
    m_vi_Vertices.push_back(m_vi_Edges.size());

    for (int i = 0; i < row; i++) {
        m_vi_Edges.insert(m_vi_Edges.end(), nodeList[i].begin(),
                          nodeList[i].end());
        m_vi_Vertices.push_back(m_vi_Edges.size());
    }
