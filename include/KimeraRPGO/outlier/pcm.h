...
class Pcm : public OutlierRemoval {
  public:
    Pcm(double                   threshold1,
        ...
        : OutlierRemoval(), threshold1_(threshold1), threshold2_(threshold2),
          incremental_(incremental), special_symbols_(special_symbols),
          total_lc_(0), total_good_lc_(0)
    {
  ...
  public:
    ...
    {
        return total_good_lc_;
    }
    size_t getNumOdomFactors()
    {
        return nfg_odom_.size();
    }
    size_t getNumSpecialFactors()
    {
        return nfg_special_.size();
    }
                        ...
                        gtsam::Values* output_values) override
    {
        ...
        // Check values to initialize a trajectory in odom_trajectories if
        // needed
        ...
        for (size_t i = 0; i < new_factors.size(); i++) {
            if (NULL == new_factors[i])
                continue;
            // we first classify the current factors into the following
            // categories:
            FactorType type = FactorType::UNCLASSIFIED;
            // current logic: The factors handled by outlier rejection are the
            // between factors. Between factors can be classified broadly into
            // odometry, loop closures, and landmark observations
            ...
            if (boost::dynamic_pointer_cast<gtsam::BetweenFactor<poseT>>(
                ...
                if (isSpecialSymbol(from_symb.chr()) ||
                    isSpecialSymbol(
                        to_symb.chr())) {  // is it a landmark observation
                    // if includes values, it is the first observation
                    if (new_values.exists(from_key) ||
                        new_values.exists(to_key)) {
                    ...
                    // Note that here we assume we receive odometry
                    // incrementally
                ...
                type = FactorType::NONBETWEEN_FACTORS;  // unrecognized factor,
                                                        // not checked for
                                                        // outlier rejection
            }
            ...
            switch (type) {
                ...
                case FactorType::FIRST_LANDMARK_OBSERVATION:  // landmark
                                                              // measurement,
                                                              // initialize
                {
                    if (debug_)
                ...
                case FactorType::LOOP_CLOSURE: {
                    if (new_factors[i]->front() != new_factors[i]->back()) {
                        // add the the loop closure factors and process them
                        // together
                ...
                default:  // the remainders are specical loop closure cases,
                          // includes the "UNCLASSIFIED" case
                {
                    nfg_special_.add(new_factors[i]);
                    do_optimize = true;
                }
        ...
        if (loop_closure_factors.size() > 0) {
            ...
            // Find inliers with Pairwise consistent measurement set
            // maximization
            do_optimize = true;
        }
        *output_nfg = buildGraphToOptimize();
        ...
        if (debug_ || do_optimize)
            log<INFO>("PCM spin took %1% milliseconds. Detected %2% total loop "
                      "closures "
                      "with %3% inliers. ") %
                spin_duration.count() % total_lc_ % total_good_lc_;
        if (log_output_) {
            saveAdjacencyMatrix(log_folder_);
            logSpinStatus(spin_duration.count(), max_clique_duration.count(),
                          log_folder_);
        }
        return do_optimize;
    }  // end reject outliers
                               ...
                               gtsam::NonlinearFactorGraph* updated_factors)
    {
        ...
        if (loop_closures_[id].factors.size() < 2) {
            ...
            loop_closures_[id].dist_matrix =
                loop_closures_[id].dist_matrix.block(0, 0, numLC - 1,
                                                     numLC - 1);
            ...
            // update inliers, or consistent factors, according to max clique
            // result
     ...
     */
    void
    removePriorFactorsWithPrefix(const char&                  prefix,
                                 gtsam::NonlinearFactorGraph* updated_factors)
    {
        ...
        // Iterate and pick out non prior factors and prior factors without key
        // with prefix
  ...
  protected:
        ...
        std::unordered_map<ObservationId, size_t>* num_new_loopclosures)
    {
        for (size_t i = 0; i < new_factors.size(); i++) {
            ...
            if (boost::dynamic_pointer_cast<gtsam::BetweenFactor<poseT>>(
                ...
                // in this case we should run consistency check to see if loop
                // closure is good
                // * odometric consistency check (will only compare against
                // odometry
                ...
                if (!output_values.exists(nfg_factor.keys().front()) ||
                    ...
                    log<WARNING>(
                        "Cannot add loop closure with non-existing keys");
                    continue;
                }
                ...
                if (isSpecialSymbol(symbfrnt.chr()) ||
                    ...
                    if (symbfrnt.chr() == symbback.chr()) {
                        odom_consistent =
                            isOdomConsistent(nfg_factor, &odom_dist);
                    }
                    else {
                        // odom consistency check only for intrarobot loop
                        // closures
                    ...
                    if (odom_consistent) {
                        ...
                        // detect which inter or intra robot loop closure this
                        // belongs to
                        ...
                        if (debug_)
                            log<WARNING>("Discarded loop closure (inconsistent "
                                         "with odometry)");
                        continue;  // discontinue since loop closure not
    ...
    void updateOdom(const gtsam::NonlinearFactor::shared_ptr& new_factor,
                    const gtsam::Values&                      output_values)
    {
        // here we have values for reference checking and initialization if
        // needed
                          ...
                          double*                            dist)
    {
        ...
        if (symb_i.chr() != symb_j.chr()) {
            log<WARNING>("Only check for odmetry consistency for intrarobot "
                         "loop closures");
        }
     ...
     */
    /*
     * pairwise loop consistency check specialized to the PoseWithCovariance
     * class
     ...
     * version) inputs are 2 loop closures (a,b) and (c,d), where a,b,c,d are
     * keys
     */
                            ...
                            const gtsam::BetweenFactor<poseT>& factor)
    {
        ...
        // -- populate extra row and column by testing pairwise consistency of
        // new lc against all previous ones
        // -- compute max clique (done in the findInliers function)
        // -- add loops in max clique to a local variable nfg_good_lc (done in
        // the updateOutputGraph function) Using correspondence rowId (size_t,
        // in adjacency matrix) to slot id (size_t, id of that lc in nfg_lc)
        if (loop_closures_.find(id) == loop_closures_.end()) {
            // does not exist yet, add
            Measurements new_measurements;
            loop_closures_[id] = new_measurements;
        }
        size_t num_lc = loop_closures_[id]
                            .factors.size();  // number of loop closures so far,
        ...
        if (num_lc > 1) {
            ...
            // now iterate through the previous loop closures and fill in last
            // row + col of adjacency
            for (size_t i = 0; i < num_lc - 1;
                ...
                double mah_distance;
                bool   consistent =
                    areLoopsConsistent(factor_i, factor, &mah_distance);
    ...
    void incrementLandmarkAdjMatrix(const gtsam::Key& ldmk_key)
    {
        ...
        size_t num_lc =
            landmarks_[ldmk_key].factors.size();  // number measurements
        Eigen::MatrixXd new_adj_matrix = Eigen::MatrixXd::Zero(num_lc, num_lc);
        Eigen::MatrixXd new_dst_matrix = Eigen::MatrixXd::Zero(num_lc, num_lc);
        if (num_lc > 1) {
            // if = 1 then just initialized
            ...
            for (size_t i = 0; i < num_lc - 1; i++) {
                ...
                // factors are (i,l) and (j,l) and connect poses i,j to a
                // landmark l
                ...
                if (symb_i.chr() != symb_j.chr()) {
                    log<WARNING>("Attempting to get odometry between different "
                                 "trajectories");
                }
                T<poseT> i_odom_j =
    ...
    {
        if (debug_)
            log<INFO>("total loop closures registered: %1%") % total_lc_;
        total_good_lc_ = 0;
        ...
        if (debug_)
            log<INFO>("number of inliers: %1%") % total_good_lc_;
    }
        ...
        const std::unordered_map<ObservationId, size_t>& num_new_loopclosures)
    {
        ...
        while (new_lc_it != num_new_loopclosures.end()) {
            ...
            size_t num_inliers = findMaxCliqueHeuIncremental(
                loop_closures_[robot_pair].adj_matrix, new_lc_it->second,
                prev_maxclique_size, &inliers_idx);
            // update inliers, or consistent factors, according to max clique
            // result num_inliers will be zero if the previous inlier set should
            // not be changed
            if (num_inliers > 0) {
    ...
    gtsam::NonlinearFactorGraph buildGraphToOptimize()
    {
        ...
        // important for gnc that we add the "special" non lc no odom factors
        // second
