...
template <class T> struct PoseWithCovariance
{
    ...
    explicit PoseWithCovariance(const gtsam::BetweenFactor<T>& between_factor)
    {
        ...
        if (std::isnan(covar.block(0, 0, r_dim, r_dim).trace())) {
            ...
            // ... later you invert this matrix, which now contains a bunch of
            // zero (it is not full rank)
    ...
    PoseWithCovariance between(const PoseWithCovariance& other) const
    {
        ...
        gtsam::Matrix         Ha, Hb;
        out.pose =
            pose.between(other.pose, Ha, Hb);  // returns between in a frame
        ...
        if (!pos_semi_def) {
            ...
            //   log<WARNING>("Warning: Covariance matrix between two poses not
            //   PSD");
            // }
        }
        if (!rotation_info || !other.rotation_info)
