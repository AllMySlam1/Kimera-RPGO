...
GenericSolver::GenericSolver(Solver            solvertype,
    ...
    : values_(gtsam::Values()), nfg_(gtsam::NonlinearFactorGraph()),
      solver_type_(solvertype), special_symbols_(special_symbols), debug_(true),
      log_(false)
{
}
                           ...
                           const gtsam::FactorIndices&        factorsToRemove)
{
    ...
    if (process_lc || remove_factors) {
        ...
        if (solver_type_ == Solver::LM) {
            ...
            values_ = gtsam::LevenbergMarquardtOptimizer(nfg_, values_, params)
                          .optimize();
        }
            ...
            values_ =
                gtsam::GaussNewtonOptimizer(nfg_, values_, params).optimize();
        }
        else {
...
void GenericSolver::removeFactorsNoUpdate(gtsam::FactorIndices factorsToRemove)
{
    // remove factors
    for (size_t index : factorsToRemove) {
        nfg_[index].reset();
...
void GenericSolver::removeLastFactor()
{
...
void GenericSolver::removePriorsWithPrefix(const char& prefix)
{
    ...
    for (auto factor : nfg_copy) {
        if (boost::dynamic_pointer_cast<gtsam::PriorFactor<gtsam::Pose3>>(
                factor)) {
