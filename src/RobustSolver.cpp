      ...
      gnc_num_inliers_(0)
{
    switch (params.outlierRemovalMethod) {
        ...
        case OutlierRemovalMethod::PCM2D: {
            outlier_removal_ = KimeraRPGO::make_unique<Pcm2D>(
                params.pcm_odomThreshold, params.pcm_lcThreshold,
                params.incremental, params.specialSymbols);
        } break;
        case OutlierRemovalMethod::PCM3D: {
            outlier_removal_ = KimeraRPGO::make_unique<Pcm3D>(
                params.pcm_odomThreshold, params.pcm_lcThreshold,
                params.incremental, params.specialSymbols);
        } break;
    ...
    switch (params.verbosity) {
        ...
        default: {
            log<WARNING>(
                "Unrecognized verbosity. Automatically setting to UPDATE. ");
        }
    }
    ...
    if (params.log_output) {
        if (outlier_removal_)
            outlier_removal_->logOutput(params.log_folder);
        log_                   = true;
    ...
    if (solver_type_ == Solver::LM) {
        ...
        if (params_.use_gnc_ && outlier_removal_) {
            ...
            std::vector<size_t> known_inlier_factor_indices(
                num_odom_factors + num_special_factors);
            std::iota(std::begin(known_inlier_factor_indices),
                      std::end(known_inlier_factor_indices), 0);
            ...
            switch (params_.gnc_threshold_mode_) {
                ...
                case (params_.GncThresholdMode::PROBABILITY):
                    gnc_optimizer.setInlierCostThresholdsAtProbability(
                        params_.gnc_inlier_threshold_);
                    break;
                default: log<WARNING>("Unsupported GNC threshold mode. ");
            }
            ...
            if (debug_) {
                log<INFO>("GNC optimize took %1% milliseconds. %2% loop "
                          "closures with "
                          "%3% inliers. ") %
            ...
            values_ =
                gtsam::LevenbergMarquardtOptimizer(nfg_, values_, lmParams)
                    .optimize();
        }
    }
