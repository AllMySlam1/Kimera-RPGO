...
TEST(Trajectory, MonteCarlo)
{
    ...
    gtsam::Matrix init_covar =
        Eigen::MatrixXd::Zero(6, 6);  // initialize as zero
...
TEST(Trajectory, PoseWithCovarianceBetween)
{
    ...
    // Eigen::MatrixXd expected_covar = Eigen::MatrixXd::Zero(6,6); // TODO
    // (Yun) fix this expected_covar.row(0) << 0.41, -0.1, 0, 0, 0, 0.1;
    ...
    EXPECT(gtsam::assert_equal(expected_between,
                               test_traj.getBetween(a1, b1).pose));
