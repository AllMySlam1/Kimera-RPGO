...
void buildTestGraph(gtsam::NonlinearFactorGraph* factors, gtsam::Values* values)
{
    /*
               <-
    a -->-->-->-A
    ...
    factors->add(gtsam::PriorFactor<gtsam::Pose3>(init_key_a, gtsam::Pose3(),
                                                  noisePriorA));
    ...
    for (size_t i = 0; i < 3; i++) {
        ...
        factors->add(gtsam::BetweenFactor<gtsam::Pose3>(key_a_prev, key_a_new,
                                                        odom, noiseOdom));
        factors->add(gtsam::BetweenFactor<gtsam::Pose3>(key_b_prev, key_b_new,
                                                        odom, noiseOdom));
    }
    ...
    for (size_t i = 3; i < 5; i++) {
        ...
        R.row(2) << 0, 0, 1;
        gtsam::Pose3 odom =
            gtsam::Pose3(gtsam::Rot3(R), gtsam::Point3(1, 0, 0));
        static const gtsam::SharedNoiseModel& noiseOdom =
            gtsam::noiseModel::Isotropic::Variance(6, 10);
        gtsam::Key key_a_prev = gtsam::Symbol('a', i);
        gtsam::Key key_a_new  = gtsam::Symbol('a', i + 1);
...
TEST(RobustSolver, RemoveLastLoopClosureNoOR)
{
    ...
    EXPECT(gtsam::assert_equal(
        ...
        est.at<gtsam::Pose3>(gtsam::Symbol('b', 0)), 0.01));
    EXPECT(gtsam::assert_equal(
        gtsam::Pose3(gtsam::Rot3(0, 0, 0, 1), gtsam::Point3(4, 1, 0)),
        est.at<gtsam::Pose3>(gtsam::Symbol('a', 5)), 10e-6));
    ...
    factors.add(gtsam::BetweenFactor<gtsam::Pose3>(
        gtsam::Symbol('a', 5), gtsam::Symbol('a', 3),
        gtsam::Pose3(gtsam::Rot3(0, 0, 0, 1), gtsam::Point3(1, 0, 0)), noise));
    pgo->update(factors);
    nfg = pgo->getFactorsUnsafe();
    est = pgo->calculateEstimate();
