...
TEST(PcmDoOptimize, LoopClosure)
{
    ...
    factors = gtsam::NonlinearFactorGraph();  // reset
    factors.add(
        gtsam::BetweenFactor<gtsam::Pose3>(2, 0, gtsam::Pose3(), noise));
    bool do_optimize =
        pcm->removeOutliers(factors, gtsam::Values(), &nfg, &est);
...
TEST(PcmDoOptimize, Beacon)
{
    ...
    gtsam::Values init_vals;
    init_vals.insert(0, gtsam::Pose3());
    bool do_optimize = pcm->removeOutliers(gtsam::NonlinearFactorGraph(),
                                           init_vals, &nfg, &est);
